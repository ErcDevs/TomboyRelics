<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
      ...
      <!-- Modal panel -->
      <div class="relative transform overflow-hidden rounded-xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg max-w-md mx-4">
            ...
            <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left w-full">
              ...
              <div class="mt-4">
                <p class="text-sm text-gray-600">
                  Save your shipping address, track orders, and get exclusive relic updates.
                </p>
              </div>
            </div>
          </div>
        ...
        <div class="bg-gray-50 px-6 py-4 sm:flex sm:flex-row-reverse sm:px-6">
          <button
            @click="$emit('close')"
            class="inline-flex w-full justify-center rounded-lg bg-ore-gold px-6 py-3 text-base font-semibold text-black shadow-sm hover:bg-ore-gold/90 sm:ml-3 sm:w-auto sm:text-sm transition-colors"
          >
            Create Account
          </button>
          <button
